...
TIME_TYPE total_system_time;   /*total wait time                  */
...
char* argv[];           /* argument list */
{
    ...
    */
    ...
    // printf("%f, %f, %f, %ld\n", arrive_rate, service_rate, sim_end, start_level);
...
long int* begin;	/* starting queue level */
{
    ...
    printf("\nPlease enter the service rate of queue? ");
...
in_system()
{
    ...
    total_system_time += (clock - sys_last_changed) * (total_customer);
    if (total_customer > 0) {
        total_wait_time += (clock - sys_last_changed) * (total_customer - 1);
    }

    if (total_customer > 0)	/*If the queue isn't empty, add to busy time... */
        total_busy_time += (clock - sys_last_changed);

    else	/* ... but if it is empty, start a service event going */
        insert_event((clock + mexpntl(1.0 / service_rate)), 1);

    sys_last_changed = clock; /* Update the time marker */
    ++total_customer;	 /* Increment total customer count	*/
    /* Keep the arrival stream of messages rolling */
    insert_event((clock + mexpntl(1.0 / arrive_rate)), 0);
...
out_system()
{
    ...
    sys_last_changed = clock;	/* Update the time marker */
    --total_customer;		/* Decrement total customer count	*/
    if (total_customer > 0)	/* If there are still some customers in the
               queue, launch another service event  */
        insert_event((clock + mexpntl(1.0 / service_rate)), 1);
...
init_rand()
{
    ...
    long  int x1, x2, x3, i, temp, __srand();
